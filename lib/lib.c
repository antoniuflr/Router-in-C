...
int read_rtable(const char *path, struct route_table_entry *rtable)
{
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		rtable[j].mask_len = mask_length(rtable[j].mask);
		j++;
	}
	return j;
}
...
int cmp_prefix(const void *a, const void *b) {
	const struct route_table_entry *entryA = (struct route_table_entry *)a;
	const struct route_table_entry *entryB = (struct route_table_entry *)b;

	if (entryA->mask_len > entryB->mask_len) return -1;
	if (entryA->mask_len > entryB->mask_len) return 1;

	if (entryA->mask_len == entryB->mask_len) {
...
struct route_table_entry *search_routes(struct route_table_entry *rtable, int start, int end, uint32_t dest_ip) {
	if (start > end)
		return NULL;
	...
	if ((dest_ip & rtable[mid].mask) < rtable[mid].prefix)
		return search_routes(rtable, start, mid - 1, dest_ip);
	else
		return search_routes(rtable, mid + 1, end, dest_ip);
