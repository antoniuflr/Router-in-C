...
/* Returns number of 1's in mask */
int mask_length(uint32_t mask);

/* Compare function for sorting the routing table 
 * First sorted by prefix length so more specific routes are first 
 * then sorted by prefix value 
 * */
int cmp_prefix(const void *a, const void *b);

/* Binary searches all entries 
 * returns 1 and places hop in next_hop or 0 if not */
